...
export type SpendingCategory = { id:string; name:string; color:string; icon_key:string }
...
const defaultCategories = [{name:'Food',color:'#c7e8a3',icon_key:'food'},{name:'Transport',color:'#a8d8e8',icon_key:'transport'},{name:'Study',color:'#c9b8ee',icon_key:'study'},{name:'Bills',color:'#f3cb85',icon_key:'bills'},{name:'Fun',color:'#f2a9b8',icon_key:'fun'},{name:'Other',color:'#c8cbc5',icon_key:'other'}]
export async function loadSpendingCategories(userId:string) {
  const client=requireClient();let {data,error}=await client.from('spending_categories').select('id,name,color,icon_key').order('created_at');if(error)throw error
  if(!data?.length){const made=await client.from('spending_categories').insert(defaultCategories.map(c=>({...c,user_id:userId}))).select('id,name,color,icon_key');if(made.error)throw made.error;data=made.data}
  return (data||[]) as SpendingCategory[]
}
export async function createSpendingCategory(userId:string,name:string,color:string,icon_key:string){const {data,error}=await requireClient().from('spending_categories').insert({user_id:userId,name,color,icon_key}).select('id,name,color,icon_key').single();if(error)throw error;return data as SpendingCategory}
export async function updateSpendingCategory(category:SpendingCategory,changes:Pick<SpendingCategory,'name'|'color'|'icon_key'>){const client=requireClient();const oldName=category.name;const updated=await client.from('spending_categories').update(changes).eq('id',category.id);if(updated.error)throw updated.error;if(changes.name!==oldName){const moved=await client.from('expenses').update({category:changes.name}).eq('category',oldName);if(moved.error){await client.from('spending_categories').update({name:oldName,color:category.color,icon_key:category.icon_key}).eq('id',category.id);throw moved.error}}}
